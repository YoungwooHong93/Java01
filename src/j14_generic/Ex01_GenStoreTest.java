...
public class Ex01_GenStoreTest {
	...
	public static void main(String[] args) {
		...
		s1.setData(12345);	 			// Integer 로 자동 형변환
		s1.setData(123.456);			// Double 로 자동 형변환
		// int -> Integer (자동형변환)
		// double -> Double (자동형변환)
		// 모든 기본자료형 -> 해당 Wrapper Class 와 자동형변환
		 s1.setData("홍길동");

		String s = (String)s1.getData();
		System.out.println(" 1. 기존방식 Test s => "+ s);
		
		s1.setData(12345); // -> OK
		Integer i = (Integer)s1.getData();
		System.out.println(" 1. 기존방식 Test i => "+ i);
		// => java.lang.ClassCastException : 런타임오류 (그러나 컴파일 오류는 없음)
		...
		sg.setData("nonGeneric 처럼 작동함");
		sg.setData(12345);
		sg.setData(new Ex04_HashSet());
		
		StoreG<String> sg1 = new StoreG<String>();
		sg1.setData("Generic 처럼 작동함");
		//sg1.setData(12345);	// 컴파일 오류
		//sg1.setData(new Ex04_HashSet());	// 컴파일 오류
		System.out.println(" 2.1 Generic 방식 => "+sg1.getData());
		
		// 2.2) T : Integer
		StoreG<Integer> sg2 = new StoreG<Integer>();
		sg2.setData(12345);
		//sg2.setData("12345"); // Type 안맞으면 컴파일 오류
		System.out.println(" 2.2 Generic 방식 => "+sg2.getData());
		
		// 2.3) T : Ex04_HashSet
		StoreG<Ex04_HashSet> sg3 = new StoreG<Ex04_HashSet>();
		sg3.setData(new Ex04_HashSet());
		System.out.println(" 2.3 Generic 방식 => "+sg3.getData());
		
		// 2.4) T : Ex05_Person
		StoreG<Ex05_Person> sg4 = new StoreG<Ex05_Person>();
		sg4.setData(new Ex05_Person("880808-2022333", "홍길순"));
		System.out.println(" 2.4 Generic 방식 => "+sg4.getData());
		
		// 2.5) T : Student
		StoreG<Student> sg5 = new StoreG<Student>();
		sg5.setData(new Student("880808-2022333", "홍길순", 90, 95));
		System.out.println(" 2.5 Generic 방식 => "+sg5.getData());
