...
public class Ex06_StringBuffer {
	...
	public static void main(String[] args) {
		// ** StringBuffer 기본 특징
		// => 기본 용량은 16 이고 필요시 자동으로 늘려줌 
		// => 생성시 용량 지정 가능 (도중에 늘리는것 보다는 충분한 용량을 지정하는것이 바람직함.)
		StringBuffer sb1 = new StringBuffer("abc");
		StringBuffer sb2 = new StringBuffer("abc");
		StringBuffer sb = new StringBuffer(10); // 생성시 용량 지정 가능
		...
		// ** StringBuffer 메서드
		System.out.println("** StringBuffer 메서트 Test **");
		s1="우리나라";
		s2="대한만국";
		sb1.append(s1);
		sb1.append(s2);
		...
		sb3.append(100);
		sb3.append(200).append(300).append(400.567); // 숫자연산을 하지는 않음(연결만)
		System.out.println("** After append sb3 => "+sb3);
		
		// => insert(값을 넣을 인덱스 번호, "변경 값")
		System.out.println("** insert sb3 => "+sb3.insert(3,"abcd"));

		// => replace(시작위치~부터 , 끝지점~까지, "변경값") : . 부터 끝까지를 END 로 변경
		System.out.println("** replace sb3 => "+sb3.replace(sb3.lastIndexOf("."),sb3.length(),"END"));
		
		// => deleteCharAt(해당 위치)
		System.out.println("** deleteCharAt sb3 => "+sb3.deleteCharAt(5)); // 5 해당 위치
		
		// => delete(~부터, ~까지)
		System.out.println("** delete sb3 => "+sb3.delete(0,3)); 		// 구간 0 부터 3 이전까지 삭제
		
		// => capacity : 용량 (byte)
		System.out.println("** capacity sb3 => "+sb3.capacity()); 		// 용량 (byte)
