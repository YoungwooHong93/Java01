...
class Student1 {
	...
	// 2) 변수를 초기화 하는 생성자 필요
	Student1(String name, int grade, int number, int java, int html, int js) {
		this.name = name;
		this.grade = grade;
		this.number = number;
		this.java = java;
...
public class Ex99_jung6_2 {
	...
	public static void main(String[] args) {
		...
		Student1 s2 = new Student1("김길동", 1, 2, 100, 60, 76);
		System.out.println("** toString Test => \n"+ s2);

		Student1 s3 = new Student1("이길동", 1, 3, 100, 60, 76);
		System.out.println("** toString Test => \n"+ s3);

		// ** 반 별 통계
		// => 배열 적용하기
		//String[] ss = { " " };
		Student1[] ss = { s1, s2, s3, new Student1("박길동", 1, 4, 100, 60, 76) };
